...
<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/20 backdrop-blur-xs p-4 font-sans text-slate-800">
    <div class="bg-white w-full max-w-4xl rounded-xl shadow-2xl overflow-hidden border border-slate-200 animate-in fade-in zoom-in duration-200">
      ...
      <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-white">
        ...
        <button @click="handleClose" class="text-slate-400 hover:text-slate-600 transition cursor-pointer p-1 rounded-lg hover:bg-slate-100">
      ...
      <div class="p-8">
        ...
        <form @submit.prevent="handleSave" class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-5">
          ...
          <div class="flex items-center">
            ...
            <div class="flex items-center space-x-2">
              <button 
                ...
                :class="form.priority === 'basic' ? 'bg-slate-700 text-white border-slate-700' : 'bg-slate-50 text-slate-600 border-slate-200'"
              >
                Basic
              </button>
              <button 
                ...
                class="px-2.5 py-1 text-xs rounded border transition cursor-pointer"
                :class="form.priority === 'normal' ? 'bg-blue-600 text-white border-blue-600' : 'bg-slate-50 text-slate-600 border-slate-200'"
              >
                Normal
              </button>
              <button 
                ...
                class="px-2.5 py-1 text-xs rounded border transition cursor-pointer"
                :class="form.priority === 'Urgent' ? 'bg-rose-600 text-white border-rose-600' : 'bg-slate-50 text-slate-600 border-slate-200'"
              >
                Urgent
              </button>
            </div>
          ...
          <div class="flex items-start md:col-span-2">
            <label class="w-28 text-xs font-semibold text-slate-700 shrink-0 pt-2">
      ...
      <!-- Modal Footer -->
      <div class="px-8 py-4 bg-slate-50 border-t border-slate-100 flex items-center space-x-3">
        <button 
          @click="handleSave" 
          :disabled="loading"
